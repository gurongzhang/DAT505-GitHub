...
********************
 ...
 3. The next step is one **key step**, for which the concept of *FOR LOOP* needs to be explained:
    To my understanding, everthing that written in this loop happened once, everytime we runs the loop, one more result is added to the scene, for example, in this exercise, the first time we run the loop, mesh one, aka cube0(X:-10, Y:-10,Z:-10)is added to the scene, the second time we run it, cube1(X:-10, Y:-10,Z:-5) is added to the scene,etc. So what we saw on the scene is the result of 64 times the loop runs. But since everthing just happened for one time, this loop can not remember the history or I want to say record the results of each loop, so it only know what happened for the last loop. As I set this clear, it is easy to understand that if I put the rotation code inside the for loop, there would be just one cube that is rotating which is the last cube-cube63.
 4. To solve this problem, the **Array** is the key point now come on stage： 
